...
class PlanPhase<R : CPGRequestResource<*>>(repository: WorkflowRepository, r: R) :
  ...
  companion object {
    fun <R : CPGRequestResource<*>> prepare(inputPhase: Phase): Result<R> = runCatching {
      ...
      check(inputRequest.getIntent() == Intent.PROPOSAL) { "Plan can't be created for a request with ${inputRequest.getIntent()} intent." }
      ...
      inputRequest.copy(id = Uuid.random().toString(), status = Status.DRAFT, intent = Intent.PLAN) as R
    }
    ...
    suspend fun <R : CPGRequestResource<*>> initiate(
      ...
      check(inputPhase.getPhaseName() == Phase.PhaseName.PROPOSAL) { "A Plan can't be started for a flow in ${inputPhase.getPhaseName().name} phase." }
      ...
      val basedOnResource: Resource? = repository.read(draftPlan.resourceType, currentPhase.request.logicalId!!)
      ...
      require(basedOnRequest.getIntent() == Intent.PROPOSAL) { "Plan can't be based on a request with ${basedOnRequest.getIntent()} intent." }
      require(basedOnRequest.getStatus() == Status.ACTIVE) { "Plan can't be based on a request with ${basedOnRequest.getStatusCode()} status." }
      require(draftPlan.getIntent() == Intent.PLAN) { "Input request has '${draftPlan.getIntent()}' intent." }
      require(draftPlan.getStatus() in AllowedStatusForPhaseStart) { "Input request is in ${draftPlan.getStatusCode()} status." }
