...
class CPGTaskRequest(resource: Task) : CPGRequestResource<Task>(TaskStatusMapper) {
  ...
  override fun setIntent(intent: Intent) {
    resource = resource.copy(intent = Enumeration(value = Task.TaskIntent.fromCode(intent.code ?: "order")))
  }
  ...
  override fun setStatus(status: Status, reason: String?) {
    resource = resource.copy(
      status = Enumeration(value = Task.TaskStatus.fromCode(mapper.mapStatusToCode(status) ?: "requested")),
  ...
  private object TaskStatusMapper : StatusCodeMapper {
    override fun mapCodeToStatus(code: String?): Status = when (code) {
      "draft" -> Status.DRAFT
      "requested", "received", "accepted", "ready", "in-progress" -> Status.ACTIVE
      "on-hold" -> Status.ONHOLD
      "cancelled", "rejected" -> Status.REVOKED
