...
class FhirOperator(
  ...
  suspend fun generateCarePlan(
    ...
    processor.apply(planDefinition, EvaluationContext(subject = subject, variables = variables, today = today))
  ...
  suspend fun generateCarePlan(
    ...
    val pd = resolver.resolvePlanDefinition(planDefinitionCanonical)
  ...
  fun evaluateMeasure(measureUrl: String): Nothing = UnsupportedOperations.evaluateMeasure(measureUrl)
  fun evaluateLibrary(libraryUrl: String): Nothing = UnsupportedOperations.evaluateLibrary(libraryUrl)
}
