...
class CPGMedicationRequest(resource: MedicationRequest) :
  ...
  override fun setIntent(intent: Intent) {
    resource = resource.copy(
      intent = Enumeration(value = MedicationRequest.MedicationRequestIntent.fromCode(intent.code ?: "order")),
    )
  }
  ...
  override fun setStatus(status: Status, reason: String?) {
    resource = resource.copy(
      status = Enumeration(
        value = MedicationRequest.MedicationrequestStatus.fromCode(mapper.mapStatusToCode(status) ?: "unknown"),
      ),
